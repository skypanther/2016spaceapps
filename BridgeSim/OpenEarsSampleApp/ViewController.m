...
#define kLevelUpdatesPerSecond 60 // We'll have the ui update 18 times a second to show some fluidity without hitting the CPU too hard.
...
//    NSArray *firstLanguageArray = @[@"COMPUTER",@"MAKE IT SO",@"STATUS",@"REPORT",@"STATUS REPORT",@"INITIATE",@"SELF DESTRUCT",@"ABORT",@"CANCEL",@"BEAM",@"ME",@"UP",@"BEAM ME UP",@"FIRE PHOTON TORPEDOES",@"FIRE PHASERS",@"DIAGNOSTICS",@"FREEZE PROGRAM",@"END PROGRAM",@"STANDBY",@"WHAT IS THE TIME"];
    
    
    NSArray *firstLanguageArray = @[@"COMPUTER",@"STATUS REPORT",@"SELF DESTRUCT",@"RED ALERT",@"FIRE PHOTON TORPEDOES",@"FIRE PHASERS",@"MOON LANDING",@"INTRUDER ALERT"];
...
//  self.heardTextView.text = [NSString stringWithFormat:@"Heard: \"%@\"", hypothesis]; // Show it in the status box.
    self.heardTextView.text = hypothesis; // Show it in the status box.
    ...
    NSString * server =@"http://172.20.10.7:8080/";
    NSString * action = @"";
    
    //    NSArray *firstLanguageArray = @[@"COMPUTER",@"STATUS REPORT",@"SELF DESTRUCT",@"RED ALERT",@"BEAM ME UP",@"FIRE PHOTON TORPEDOES",@"FIRE PHASERS",];

    
    self.mainView.backgroundColor = [UIColor whiteColor];

    if ([hypothesis rangeOfString:@"RED"].location != NSNotFound) {action = @"RED ALERT"; self.mainView.backgroundColor = [UIColor redColor];}
    if ([hypothesis rangeOfString:@"ALERT"].location != NSNotFound) {action = @"RED ALERT";self.mainView.backgroundColor = [UIColor redColor];}
    if ([hypothesis rangeOfString:@"STATUS REPORT"].location != NSNotFound) {action = @"STATUS REPORT";}
    if ([hypothesis rangeOfString:@"MAKE IT SO"].location != NSNotFound) {action = @"MAKE IT SO";}
    if ([hypothesis rangeOfString:@"ENGAGE"].location != NSNotFound) {action = @"ENGAGE";}
    if ([hypothesis rangeOfString:@"BEAM ME UP"].location != NSNotFound) {action = @"BEAM ME UP";}
    if ([hypothesis rangeOfString:@"PHOTON"].location != NSNotFound) {action = @"FIRE PHOTON TORPEDOES";self.mainView.backgroundColor = [UIColor redColor];}
    if ([hypothesis rangeOfString:@"PHASER"].location != NSNotFound) {action = @"FIRE PHASERS";self.mainView.backgroundColor = [UIColor redColor];}
    if ([hypothesis rangeOfString:@"DESTRUCT"].location != NSNotFound) {action = @"SELF DESTRUCT";self.mainView.backgroundColor = [UIColor redColor];}
    if ([hypothesis rangeOfString:@"LANDING"].location != NSNotFound) {action = @"MOON LANDING";}
    if ([hypothesis rangeOfString:@"INTRUDER"].location != NSNotFound) {action = @"INTRUDER ALERT";self.mainView.backgroundColor = [UIColor redColor];}
    if ([hypothesis rangeOfString:@"COMPUTER"].location != NSNotFound) {action = @"COMPUTER";}
    
    NSLog(@"action = %@",action);
    
    NSString * cleanaction =  [action.lowercaseString stringByReplacingOccurrencesOfString:@" " withString:@""]; //strip spaces

    self.heardTextView.text = action;
    self.heardTextView.minimumZoomScale = 0.3;
    //    self.heardTextView.hidden = true;
    self.statusTextView.hidden = true;
    self.pocketsphinxDbLabel.hidden = true;
    self.fliteDbLabel.hidden = true;
    
    if (![action isEqualToString:@""]) {
    NSString * url = [NSString stringWithFormat:@"%@?action=%@",server,cleanaction];

        /*
    AudioServicesDisposeSystemSoundID (playsound);

    id sndpath = [[NSBundle mainBundle]
                  pathForResource: action
                  ofType:@"mp3"
                  inDirectory:@"/"];
    
    NSFileManager *fileManager = [[NSFileManager alloc] init];
    BOOL exists = [fileManager fileExistsAtPath:sndpath];
    
    if (exists) {
        NSLog(@"playing sound %@",action);

        baseURL = (CFURLRef) CFBridgingRetain([[NSURL alloc] initFileURLWithPath:sndpath]);
        AudioServicesCreateSystemSoundID (baseURL, &playsound);
    ...
    NSLog(@"ABOUT TO ACCESS async url: %@",url);
    
    dispatch_async(kBgQueue,^{
        NSData* data = [NSData dataWithContentsOfURL:asyncurl];
        [self performSelectorOnMainThread:@selector(playAsync)
...
-(void)playAsync {
    NSLog(@"completed web api request");
}
